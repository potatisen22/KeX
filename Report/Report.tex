...
\begin{document}
	...
	The controller used in this project is an LQ-controller. The controller is used to check that our mathematical model of the UAV is controllable, observable and that it can be controlled well enough to do steady flight operations.  An LQR utilizes a cost function and minimises said cost function to optimise the controller. To be able to create our LQR we first had to construct the steady state form of our model by linearising the mathematical model and rewriting it in the form of equation (\ref{eq:steady_state_1}) and (\ref{eq:steady_state_2})
	...
	To design the LQR we then use some of the constants in the steady state equation to express the Q matrix in the following way.
