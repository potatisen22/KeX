...
\begin{document}
	...
	\section{Quadcopter navigation}
		...
		Potential fields are based in the calculation, for each point in the space, of a function that is the sum of an attractive and a repulsive potential, that will condition the movement of the vehicle for one iteration. The repulsive and the attractive potentials are calculated according to \cite{potfieldsmethod}, as can be seen in equations (\ref{eq:pot_att}) and (\ref{eq:pot_rep}) respectively.
 		...
 		U\subb{rep}(\bm{q}) =  \left\lbrace  {\begin{array}{cc}
 			...
 			\end{array} } \right.
 		\end{equation}
	Where $\bm{q}$ is the vehicle's linear position in the space. $\rho\subb{goal}(\bm{q})$ and $\rho\subb{obst}(\bm{q})$ are the distances from the vehicle to the goal or to the nearest obstacle respectively, calculated with the Euclidean norm $\rho\subb{x}(\bm{q}) = \parallel \bm{q} - \bm{q\subb{x}}\parallel$. $d$ is the distance to change from parabolic to conic well as to make the attractive potential smoother, and $\rho\subb{0}$ is the radius of the sphere of influence of the UAV, as to detect near obstacles. Finally, $\xi$ and $\eta$ are positive scaling factors. \\ 
	
	After establishing the potential function, the desired force that the vehicle should make in order to advance in space according to the potential function is calculated with equation (\ref{eq:force_from_pot}), leading to equations () and () that show the calculation of this repulsive and attractive forces.
	...
	\begin{equation} \label{eq:force_rep}	
		...
		\eta (\frac{1}{\rho\subb{obst}(\bm{q})}-\frac{1}{\rho\subb{0}})(\frac{1}{\rho\subb{obst}^2(\bm{q})})\frac{(\bm{q}-\bm{q\subb{obst}})}{\parallel\bm{q}-\bm{q\subb{obst}}\parallel} & \textrm{if} \ \rho\subb{obst}(\bm{q})\leq d \\
		\\
		0 &\textrm{if} \ \rho\subb{obst}(\bm{q})> \rho\subb{0} \\
		\end{array} } \right.
	$}
