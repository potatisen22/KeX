...
\begin{document}
	...
	\section{Results}
		\subsection{Control}
			...
			\begin{figure}[h]
	  			\centering
	  			\includegraphics[width=6cm]{Results/Controll/Fig12}
...
We have implemented some different case scenarios to cover all the possible issues of the model. In the first one the number of both obstacles and UAVs generated is random, between a minimum and a maximum value. This can help us check how many drones can finally get to their goals without getting stuck, in a random environment where the local minima geometries are not corrected. The second case test how different UAVs react to each other, since they are also seen as moving obstacles.
