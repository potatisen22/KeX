...
\begin{document}
	...
	The core of this project focuses on how to make aerial vehicles fly autonomously from an initial position to a goal. Therefore, apart from the main algorithm that will make this possible, it is really important that the vehicle can acquire precise information about its condition and its surroundings. Sensors do not only have to provide information about the state of the UAV as to close the loop for the controller, but also about the objects the vehicle may encounter throughput its path, as to make the navigation safe, and prevent and block possible crashes.\\
		...
		The sensors analised are the most accessible ones because of their simplicity, price and availability of open source libraries.\\
		
		\subsubsection{ADXL345 - Triple axis accelerometer} This digital sensor measures proper acceleration along the three axes in the body frame. the static acceleration of gravity in tiltsensing applications, 
		
		\subsubsection{ ITG3205- Triple axis gyroscope} 
		
		\subsubsection{HMC5883l - Triple axis magnetometer}
	...
	The controller used in this project to check that our drone is controllable and observable is an LQ-controller. An LQR utilizes a cost function and tries to minimize said cost function. the reason behind using a LQR is that it usually has smaller errors than a normal PID-controller [SOURCE] and since the main goal of this project is to avoid colliding any of our UAVs it would be reasonable to focus on getting an error as small as possible.   
