...
\end{equation}
	...
	\begin{figure}[H]
		...
		As can be seen in \figurename{ \ref{fig:env_1}}, \figurename{ \ref{fig:env_2}} and \figurename{ \ref{fig:env_3}}, each UAV (represented by a colour) reaches its goal (represented by a $\blacklozenge$ of the same colour) while avoiding the obstacles that it encounters throughout its path. In \figurename{ \ref{fig:goal}} we can see the distance from the vehicle to its goal at each time step. As can be seen in \figurename{ \ref{fig:obst}}, the distance between a the surface of a UAV and its closest obstacle's surface (static or dynamic) its always above zero. In \figurename{ \ref{fig:vel}}, we can see the velocity for all UAVs in each time step.
